<template>
    <div class="m-home">
        <div v-for="(item, index) in listData" :key="index">
            <h3 class="video-list-item-title">{{item.title.name}}</h3>
            <div class="video-list active">
                <div class="video-item-list">
                    <router-link
                        :to="{ path: '/videoview', query: { item: JSON.stringify(item) } }"
                        tag="a"
                        href="#"
                        class="video-item"
                    ...
                    >
                        ...
                        <div class="card">
                            <img v-lazy="item.imageUrl.indexOf('http')!=-1?'http://192.168.0.213:3000/api/aiqiyi/pic?url=' + item.imageUrl:'http://192.168.0.213:3000/api/aiqiyi/pic?url=https:' + item.imageUrl" />
                            <div class="count">
                                <div>{{ item.meta }}</div>
                            </div>
                        </div>
